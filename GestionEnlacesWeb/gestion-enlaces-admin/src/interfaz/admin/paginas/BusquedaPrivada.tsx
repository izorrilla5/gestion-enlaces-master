...
import { getCategorias, getMedicos, getTags, getTipos } from 'infraestructura/auth/app-data-manager';
...
import ListaEnlacesPrivado from "./ListaEnlacesPrivado";
...
export interface BusquedaPrivada_DispatchProps {
...
const mapDispatchToProps = (dispatch: React.Dispatch<any>) => ({
  ...
  loadInitalData: (idIdiomaUsuario?: number) => {
    dispatch(
      search({ titulo: "", url: "", categoria: undefined, votos: [], tags: [] }, 1, idIdiomaUsuario)
    );
  },
});

type BusquedaPrivadaProps = IEstadoAplicacion & BusquedaPrivada_DispatchProps;

const _BusquedaPrivada = (props: BusquedaPrivadaProps) => {
  //Configuracion i18n
  ...
  const [tituloBusqueda, setTituloBusqueda] = React.useState<string | '' >(
  ...
  const [usuarioBusqueda, setUsuarioBusqueda] = React.useState<IUsuario | null>(
    props.datosEnlaces.enlaceBusqueda != undefined &&
      props.datosEnlaces.enlaceBusqueda.usuario != undefined
      ? props.datosEnlaces.enlaceBusqueda.usuario
      : null
  );
  ...
  const searchEnlace = (tituloLive?: string, page?: number) => {
  ...
  const searchTagFromListItem = (tag: ITag) => {
    ...
    props.doSearch(enlace, undefined, getLoggedUserData().idIdiomaSeleccionado);
  };
...
export const BusquedaPrivada = connect(
  mapStateToProps,
  mapDispatchToProps
)(_BusquedaPrivada);

export default BusquedaPrivada;
